function decisecond(){
    if(get('game-mode').value==1&&parseFloat(get('time').innerHTML)<=0&&get('max-time').value>0){
        /*if in max-time mode and time is less than or equal to 0 and max-time isn't 0*/
        stop()
    }else if(get('game-mode').value==0&&get('max-points').value!=0&&parseInt(get('score').innerHTML)>=get('max-points').value){
        ...
        get('time').innerHTML=((parseFloat(get('time').innerHTML)+((get('game-mode').value==1&&get('max-time').value>0)?-.1:.1)).toFixed(1))
    }
}
function get(i){
...
function randomize_buttons(clicked_button_id){
    ...
    if(p){/*if game has not ended with this click*/
        var ti=0;
...
function reset(){
    if(confirm('Reset settings?')){
        ...
        get('score').value=get('time').value=get('y-margin').value=get('red-onclick').value=0;
        get('game-mode').value=get('green-frequency').value=get('green-points').value=get('audio-volume').value=get('red-frequency').value=get('red-points').value=1;
...
function save(){
    ...
    i=11;
    j=['grid-dimensions','game-mode','max-points','max-time','green-frequency','green-points','red-frequency','red-onclick','red-points','audio-volume','y-margin','start-key'];
    do{
        if(get(j[i]).value==[5,1,50,30,1,1,1,0,1,1,0,'H'][i]){
            ls.removeItem('speedbutton'+i)
        }else{
            ls.setItem('speedbutton'+i,get(j[i]).value)
        }
    }while(i--);
    j=0
}
function set_settings_disable(i){
    ...
    get('grid-dimensions').disabled=get('game-mode').disabled=get('green-frequency').disabled=get('green-points').disabled=get('max-points').disabled=get('reset-button').disabled=get('max-time').disabled=get('red-frequency').disabled=get('red-onclick').disabled=get('red-points').disabled=i
}
function start(){
    /*verify settings are numbers and greater than or equal to 0*/
    i=7;
    var j=['max-points','max-time','green-frequency','green-points','red-frequency','red-points','audio-volume','y-margin'];
    do{
        if(isNaN(get(j[i]).value)||get(j[i]).value<0){
            get(j[i]).value=[50,30,1,1,1,1,1,0][i]
        }
    }while(i--);
    ...
    get('score-max').innerHTML=get('time-max').innerHTML='';

    /*generate green and red buttons*/
    randomize_buttons(random_number(grid_side*grid_side));

    get('score').innerHTML=get('time').innerHTML=0;
...
function setup(){
    if(get('start-button').disabled||grid_side!=get('grid-dimensions').value){
        ...
        get('grid-dimensions').value=ls.getItem('speedbutton0')===null?5:parseInt(ls.getItem('speedbutton0'));
        get('game-mode').value=ls.getItem('speedbutton1')===null?1:parseInt(ls.getItem('speedbutton1'));
        get('max-points').value=ls.getItem('speedbutton2')===null?50:parseInt(ls.getItem('speedbutton2'));
        get('max-time').value=ls.getItem('speedbutton3')===null?30:parseInt(ls.getItem('speedbutton3'));
        get('green-frequency').value=ls.getItem('speedbutton4')===null?1:parseInt(ls.getItem('speedbutton4'));
        get('green-points').value=ls.getItem('speedbutton5')===null?1:parseInt(ls.getItem('speedbutton5'));
        get('red-frequency').value=ls.getItem('speedbutton6')===null?1:parseInt(ls.getItem('speedbutton6'));
        get('red-onclick').value=ls.getItem('speedbutton7')===null?0:parseInt(ls.getItem('speedbutton7'));
        get('red-points').value=ls.getItem('speedbutton8')===null?1:parseInt(ls.getItem('speedbutton8'));
        get('audio-volume').value=ls.getItem('speedbutton9')===null?1:parseFloat(ls.getItem('speedbutton9'));
        get('y-margin').value=ls.getItem('speedbutton10')===null?0:parseInt(ls.getItem('speedbutton10'));
        if(ls.getItem('speedbutton11')===null){
            get('start-key').value='H'
        }else{
            get('start-key').value=ls.getItem('speedbutton11');
